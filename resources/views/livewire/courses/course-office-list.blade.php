<div class="space-y-6 font-cairo text-right select-none" dir="rtl">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between border-b border-[#1F2937]/60 pb-5 gap-4">
        <div>
            <h1 class="text-2xl font-bold text-[#E5E7EB] tracking-wide flex items-center gap-2">
                <span>جناح شؤون الضباط</span>
                <span class="text-sm font-normal text-gray-500 font-mono">//</span>
                <span class="text-xl font-medium text-gray-300">كشف دورات الضباط النشطة</span>
            </h1>
            <p class="text-xs sm:text-sm text-gray-400 mt-1">تخطيط ومتابعة البرامج التدريبية، الدورات التكتيكية، ومستويات اجتياز الضباط</p>
        </div>
        <div class="shrink-0">
    ...
    <!-- Relative Distribution Report (Custom Tailwind Analytics Chart) -->
    @php
        $totalRegisteredOfficers = \DB::table('course_officer')->count(); 
    @endphp
    <div class="bg-[#111827] border border-[#1F2937] rounded-2xl p-6 shadow-xl space-y-4">
        <div class="flex items-center justify-between border-b border-[#1F2937] pb-3.5">
            <h2 class="text-md font-bold text-[#E5E7EB] flex items-center gap-2">
                ...
                <span>تقرير التوزيع النسبي للضباط على الحقائب التدريبية</span>
            </h2>
            <span class="text-xs font-mono text-gray-400 bg-[#070B14] border border-[#1F2937] px-3 py-1 rounded-lg">
                إجمالي الضباط المسجلين: <span class="text-blue-400 font-bold font-mono">{{ $totalRegisteredOfficers }}</span> ضابط
            </span>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 pt-2">
            @forelse($courses->take(6) as $c) 
                @php
                    $courseCount = $c->officers()->count(); 
                    $percentage = $totalRegisteredOfficers > 0 ? round(($courseCount / $totalRegisteredOfficers) * 100) : 0;
                ...
                <div class="bg-[#070B14]/60 border border-[#1F2937]/70 p-4 rounded-xl space-y-2.5 hover:border-[#2563EB]/30 transition-all duration-200">
                    <div class="flex justify-between items-center text-xs font-bold">
                        ...
                        <div class="flex items-center gap-2 font-mono">
                            <span class="text-gray-400">({{ $courseCount }} ضابط)</span>
    ...
    <div class="bg-[#111827] border border-[#1F2937] rounded-2xl shadow-2xl overflow-hidden">
        <div class="p-5 border-b border-[#1F2937] flex items-center justify-between bg-[#111827]/50">
            <h2 class="text-md font-bold text-[#E5E7EB] flex items-center gap-2.5">
                ...
                <span>البيانات التفصيلية للحقائب التدريبية للضباط</span>
            </h2>
                    ...
                    <tr class="hover:bg-gray-800/20 transition-colors duration-150 group">
                        ...
                        <td class="p-4 text-center align-middle whitespace-nowrap">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                <button type="button" 
                                        wire:click="openOfficerModal({{ $course->id }})"
                                        title="إسناد وإضافة الضباط لهذه الدورة"
                                ...
                                <!-- Delete Button with dynamic Spinner -->
                                <button type="button" 
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-[2px] transition-all duration-200"
         wire:keydown.escape.window="closeOfficerModal">
        
        <!-- Backdrop click handler -->
        <div class="absolute inset-0 cursor-default" wire:click="closeOfficerModal"></div>
        ...
        <div class="bg-[#111827] border border-[#1F2937] w-full max-w-4xl rounded-xl shadow-2xl overflow-hidden transform transition-all relative z-10 animate-in fade-in zoom-in-95 duration-150">
            ...
            <div class="px-5 py-4 border-b border-[#1F2937] flex items-center justify-between bg-[#111827] select-none">
                <div>
                    <h3 class="text-md font-bold text-white tracking-wide">إسناد الضباط والقوة البشرية</h3>
                    <p class="text-xs text-gray-400 mt-1">الدورة: <span class="text-gray-300 font-semibold">{{ $selectedCourseName }}</span></p>
                </div>
                <button type="button" wire:click="closeOfficerModal" class="w-7 h-7 rounded-lg bg-[#111827] border border-[#1F2937] text-gray-400 hover:text-white flex items-center justify-center transition-colors">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                    </svg>
                </button>
            ...
            <div class="p-5 space-y-4">
                ...
                <div class="relative flex items-center">
                    ...
                    <input type="text" wire:model.live.debounce.250ms="searchOfficer" 
                           ...
                           placeholder="ابحث عن ضابط بالرقم العسكري، الرتبة أو الاسم...">
                </div>

                <!-- Officers Checklist -->
                <div class="space-y-2">
                    <label class="block text-xs font-bold text-gray-400 tracking-wider select-none">حدد قوة الضباط المختارين:</label>
                    ...
                    <div class="max-h-[160px] overflow-y-auto pr-0.5 pl-0.5 gap-3 grid grid-cols-1 md:grid-cols-2 scrollbar-thin scrollbar-thumb-gray-800">
                        @forelse($officerList as $officer)
                        <label class="flex items-center justify-between p-3.5 bg-[#070B14]/40 border border-[#1F2937] rounded-lg hover:border-gray-700/80 cursor-pointer transition-all duration-150 group">
                            <div class="flex flex-col text-right select-none">
                                ...
                                <span class="text-[11px] text-gray-500 font-sans mt-1">الرقم العسكري: <span class="font-mono text-gray-400">{{ $officer->military_id?? $militrary->id }}</span></span>
                            </div>
                            <input type="checkbox" value="{{ $officer->id }}" wire:model="selectedOfficerIds"
                        ...
                        <div class="text-center col-span-2 py-8 text-gray-500 text-xs">
                            ...
                            لا توجد سجلات ضباط مطابقة لخيارات البحث الحالية.
                        </div>
                        @endforelse
                    </div>
                </div>
              ...
              <div class="px-5 py-4 border-t border-[#1F2937] flex items-center justify-between bg-[#111827]/60 select-none">
                <span class="text-xs font-mono text-gray-400">
                    تم تحديد: <span class="text-emerald-400 font-bold font-mono">{{ count($selectedOfficerIds ?? []) }}</span> من الضباط
                </span>
            </div>

            <!-- Modal Footer Action Buttons -->
            <div class="px-5 py-4 border-t border-[#1F2937] flex items-center justify-start flex-row-reverse gap-3 bg-[#111827] select-none">
                <button type="button" wire:click="saveOfficers" class="inline-flex items-center justify-center bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold px-4 py-2 rounded-xl transition-colors active:scale-95 shadow-lg shadow-blue-600/10">حفظ</button>
                <button type="button" wire:click="closeOfficerModal" class="bg-[#111827] border border-[#1F2937] hover:bg-gray-800 text-gray-400 hover:text-white text-xs font-bold px-4 py-2 rounded-xl transition-colors active:scale-95">إلغاء الأمر</button>
            </div>
        </div>
    </div>
