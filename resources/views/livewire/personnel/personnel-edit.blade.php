<div x-data="{ open: @entangle('isOpen') }" 
    ...
    <div class="bg-[#111827] border border-[#1F2937] p-5 rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto font-cairo text-right" dir="rtl" @click.away="open = false">
        ...
        <div class="flex items-center justify-between mb-4 pb-2 border-b border-[#1F2937]">
            <h2 class="text-white text-base font-bold">تعديل بيانات ملف الافراد</h2>
    ...
    @enderror
            ...
            {{-- شبكة الحقول الأساسية والعسكرية والاتصال --}}
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                {{-- الاسم الكامل --}}
                <div class="space-y-1">
                    <label class="block text-xs font-semibold text-gray-400">الاسم الرباعي الكامل</label>
                    <input type="text" wire:model="full_name" class="w-full bg-[#070B14] border border-[#1F2937] rounded-lg text-sm px-3 py-2 text-[#E5E7EB] focus:border-[#2563EB] focus:outline-none transition-colors">
                    @error('full_name') <span class="text-rose-500 text-[11px]">{{ $message }}</span> @enderror
                </div>
                
                {{-- الرقم العسكري (تم تحسين الخلفية والمؤشر لتفادي اللون الأبيض تماماً) --}}
...
<!-- 1111111111111111111111111111111 -->
                    ...
                    x-init="
                        // دالة لفحص الرتبة وتحديد هل هي مخصصة أم لا
                        ...
                        // فحص القيمة فوراً عند التشغيل
                        checkRank(rankValue);

                        // مراقبة القيمة عند تغيرها أو جلبها من الـ Backend (حل مشكلة الفراغ)
                    ...
                    <div x-show="isCustomRank" x-cloak class="relative flex items-center gap-2">
                        <input type="text" 
                            ...
                            class="w-full bg-[#070B14] border border-[#2563EB] rounded-lg text-sm px-3 py-2 text-[#E5E7EB] focus:outline-none font-cairo" />
                ...
                {{-- الحالة العملياتية / الإدارية --}}
                <div class="space-y-1">
                    <label class="block text-xs font-semibold text-gray-400"> الحالة</label>
                ...
                {{-- التخصص الرئيسي --}}
                <div class="space-y-1">
                    <label class="block text-xs font-semibold text-gray-400">التخصص الرئيسي</label>
                ...
                {{-- التخصص الفرعي --}}
                <div class="space-y-1">
                    <label class="block text-xs font-semibold text-gray-400">التخصص الفرعي</label>
                    <input type="text" wire:model="sub_specialty" class="w-full bg-[#070B14] border border-[#1F2937] rounded-lg text-sm px-3 py-2 text-[#E5E7EB] focus:border-[#2563EB] focus:outline-none transition-colors">
                    @error('sub_specialty') <span class="text-rose-500 text-[11px]">{{ $message }}</span> @enderror
                </div>
                ...
                {{-- رقم الجوال --}}
                <div class="space-y-1">
                    <label class="block text-xs font-semibold text-gray-400">رقم الجوال </label>
                    <input type="tel"
                    maxlength="10"
                    oninput="this.value = this.value.replace(/[^0-9]/g, '').slice(0, 10)"
                    wire:model="phone" placeholder="05xxxxxxxx" class="w-full bg-[#070B14] border border-[#1F2937] rounded-lg text-sm px-3 py-2 text-[#E5E7EB] font-mono focus:border-[#2563EB] focus:outline-none transition-colors">
    ...
    x-data="{
        ...
        togglePicker() {
            if (!this.showPicker) {
                ...
                // حساب الموقع ليرتفع التقويم إلى الأعلى مباشرة فوق الحقل مع هوامش تمنع الالتصاق
                this.pickerTop = rect.top + window.scrollY - 340;
                
                // محاذاة الحافة اليمنى للتقويم مع الحقل ليمتد باتجاه اليسار
        ...
        // دوال القفز السريع بين السنوات بالأسهم (8 سنوات للخلف أو الأمام للوصول للتواريخ القديمة مثل 1242)
        prevYearPage() {
            this.currentYear -= 8;
            this.generateCalendar();
        },
    ...
    <div x-show="showPicker" 
        ...
        <div class="flex items-center justify-between border-b border-[#1e293b]/60 pb-2 mb-3 select-none w-full">
            <button type="button" 
                    ...
                    class="p-1 hover:bg-[#162235] rounded-md text-gray-400 hover:text-white transition-colors cursor-pointer shrink-0">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
            </button>
            
            <div @click="viewMode = viewMode === 'days' ? 'years' : 'days'" 
                 class="flex items-center justify-center gap-1 font-bold text-xs text-gray-200 font-cairo cursor-pointer hover:text-blue-400 transition-colors px-2 py-0.5 rounded hover:bg-[#162235] overflow-hidden whitespace-nowrap">
        ...
        <div x-show="viewMode === 'days'">
            <div class="grid grid-cols-7 gap-1 text-center text-[11px] font-bold text-gray-500 mb-2 select-none w-full" 
                 style="display: grid !important; grid-template-columns: repeat(7, minmax(0, 1fr)) !important; direction: rtl !important;">
            ...
            <div class="grid grid-cols-7 gap-1 w-full" 
                ...
                <template x-for="day in daysInMonth" :key="day">
                    <button type="button" 
                            @click="selectDay(day)"
                            class="block w-full text-center p-1.5 text-xs rounded-lg font-mono transition-all cursor-pointer"
                            style="aspect-ratio: 1/1 !important;"
                            :class="selectedDate && selectedDate.startsWith(currentYear + '/' + String(currentMonth + 1).padStart(2, '0') + '/' + String(day).padStart(2, '0')) ? 'bg-[#2563EB] border border-[#2563EB] text-white font-bold shadow-lg shadow-[#2563EB]/40' : 'bg-[#101726]/40 border border-[#1e293b]/50 text-gray-300 hover:bg-[#2563EB] hover:text-white'">
        ...
        <div x-show="viewMode === 'years'" 
             x-transition
             class="space-y-2 max-h-48 overflow-y-auto pr-1 scrollbar-thin">
            <div class="grid grid-cols-2 gap-2 w-full">
                <template x-for="year in availableYears" :key="year">
                    <button type="button" 
                            @click="selectYear(year)"
                            class="w-full text-center py-2 px-1 text-xs font-mono font-bold rounded-lg transition-all duration-200 cursor-pointer block border"
                            :class="currentYear === year ? 
                                    'bg-[#2563EB] text-white border-[#2563EB] shadow-md shadow-[#2563EB]/20' : 
                                    'bg-[#101726]/60 text-gray-300 border-[#1e293b]/80 hover:bg-[#162235] hover:text-white'">
...
{{-- تاريخ الترقية الحالية --}}
<div class="text-right relative block mt-4" 
    x-data = "{
        showPicker: false,
        viewMode: 'days', 
...
<!-- 0000000000000000 -->
                {{-- حقل مدة الخدمة (تعديل يدوي - سنوات وأشهر) --}}
                <div class="space-y-1.5 md:col-span-2 border-t border-[#1F2937]/60 pt-3">
                    <label class="block text-xs font-semibold text-gray-400 font-cairo">مدة الخدمة الفعلية</label>
            ...
            <div class="space-y-1">
                <label class="text-gray-400 text-xs font-semibold">ملاحظات عامة وملفات الخدمة</label>
                <textarea wire:model="notes" rows="2" class="w-full bg-[#070B14] border border-[#1F2937] rounded-lg text-sm px-3 py-2 text-[#E5E7EB] focus:border-[#2563EB] focus:outline-none"></textarea>
            ...
            <div class="flex justify-end gap-2 pt-2 border-t border-[#1F2937]">
                ...
                <button type="submit" class="bg-blue-600 text-white px-4 py-1.5 rounded-lg text-xs font-bold hover:bg-blue-700 transition-colors tracking-wide">
