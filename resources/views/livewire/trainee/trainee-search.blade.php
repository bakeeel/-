<div class="space-y-6 font-cairo text-right p-4 md:p-6 bg-[#070B14] min-h-screen text-gray-200" dir="rtl">
    ...
    <div class="bg-[#0f1626]/60 backdrop-blur-md border border-[#1e293b] rounded-xl p-6 shadow-xl">
        ...
        <h2 class="text-[#3b82f6] text-sm font-bold mb-6 flex items-center gap-3">
            ...
            <span class="tracking-wide">منظومة البحث والتقصي الذكي عن المتدربين</span>
        </h2>
            ...
            <div class="flex flex-col md:flex-row items-end gap-4 w-full">
                ...
                <div class="space-y-2 w-full md:flex-1">
                    <label class="text-xs font-bold text-gray-400 flex items-center gap-2 px-1 select-none">
                        <i class="fas fa-user text-blue-500 w-4 text-center text-xs"></i> 
                        <span>اسم المتدرب</span>
                    </label>
                    <input type="text" wire:model.defer="search_name" placeholder="أدخل اسم المتدرب رباعياً..."
                ...
                <div class="space-y-2 w-full md:flex-1">
                    <label class="text-xs font-bold text-gray-400 flex items-center gap-2 px-1 select-none">
                        <i class="fas fa-id-card text-blue-500 w-4 text-center text-xs"></i> 
                        <span>الرقم العسكري للمتدرب</span>
                    </label>
    ...
    {{-- ===== لوحة نتيجة الاستعلام وعرض ملف المتدرب ===== --}}
    @if($trainee)
    <div class="bg-[#0f1626]/40 backdrop-blur-md border border-[#1e293b] rounded-xl p-6 space-y-8 shadow-2xl">
        ...
        <div class="flex items-center justify-between border-b border-[#1e293b] pb-4">
            <h2 class="text-[#3b82f6] text-sm font-bold flex items-center gap-2">
                ...
                <span class="tracking-wide">ملف قيد المتدرب المستعلم عنه رسمياً</span>
            </h2>
            <span class="text-[11px] px-2.5 py-1 rounded-md bg-[#2563EB]/10 text-blue-400 border border-[#2563EB]/20 font-bold">جناح شؤون المتدربين</span>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
            ...
            <div class="flex flex-col items-center gap-4 text-center bg-[#070B14]/40 border border-[#1e293b] p-5 rounded-2xl">
                ...
                {{-- الأفاتار العسكري للمتدرب --}}
                <div class="relative">
                    <div class="w-32 h-40 rounded-xl bg-[#0a0f1d] border border-[#1e293b] overflow-hidden shadow-2xl">
                        @if($trainee->avatar && file_exists(storage_path('app/public/' . $trainee->avatar)))
                        ...
                        @else
                            <div class="h-full flex flex-col items-center justify-center text-gray-700 bg-[#070B14]">
                                <i class="fas fa-graduation-cap text-4xl mb-2 text-gray-600"></i>
                                <span class="text-[9px] text-gray-500">لا توجد صورة رسمية</span>
                            </div>
                        @endif
                    </div>
                    <div class="absolute -bottom-2 -right-2 bg-blue-600 p-1.5 rounded-full border-4 border-[#070B14] shadow-lg">
                        <i class="fas fa-user-graduate text-[10px] text-white"></i>
                    </div>
                </div>
                ...
                <div class="w-full bg-[#0a0f1d] border border-[#1e293b] rounded-xl py-3 px-4">
                    ...
                    <p class="text-[#3b82f6] font-mono font-bold text-xl tracking-widest">{{ $trainee->military_id }}</p>
                </div>

                {{-- الأزرار والتحكم التفاعلي بقيد المتدرب --}}
                <div class="w-full space-y-2">
                    <button
                        wire:click="$dispatch('open-edit-modal', { id: {{ $trainee->id }} })"
                        ...
                        <i class="fas fa-user-edit text-[11px]"></i> تعديل ملف المتدرب
                    </button>
                    <button class="w-full flex items-center justify-center gap-2 bg-[#1e293b] hover:bg-[#334155] text-gray-300 py-2.5 rounded-lg text-xs font-semibold transition border border-gray-700">
                        <i class="fas fa-print text-[11px]"></i> تصدير بيان المتدرب
                    </button>
                </div>

                {{-- تضمين مودال التعديل الفوري الخاص بالمتدربين ليعمل خلف الكواليس --}}
                <livewire:trainee.trainee-edit />
            </div>

            {{-- ── العمود الثاني: السجل التدريبي والبيانات الأساسية ── --}}
            <div class="space-y-3">
                @php
                    $infoItems = [
                        ['label' => 'الاسم الكامل للمتدرب', 'value' => $trainee->full_name, 'icon' => 'user'],
                        ['label' => 'الرتبة العسكرية',       'value' => $trainee->rank, 'icon' => 'star'],
                        ['label' => 'التخصص الأساسي',      'value' => $trainee->primary_specialty ?? 'غير محدد', 'icon' => 'award'],
                        ['label' => 'التخصص الفرعي',       'value' => $trainee->sub_specialty ?? 'غير محدد', 'icon' => 'layer-group'],
                        ['label' => 'رقم الهاتف المعتمد',    'value' => $trainee->phone ?? '---', 'icon' => 'phone', 'mono' => true],
                        ['label' => 'حالة القيد التدريبي',  'value' => $trainee->status ?? 'نشط', 'icon' => 'toggle-on'],
                    ];
                @endphp
            ...
            {{-- ── العمود الثالث: التواريخ العسكرية للمتدرب ── --}}
            <div class="space-y-3">
                @php
                    $dates = [
                        ['label' => 'تاريخ التعيين والالتحاق', 'date' => $trainee->appointment_date, 'icon' => 'calendar-check'],
                        ['label' => 'تاريخ التثبيت في الخدمة',  'date' => $trainee->confirmation_date, 'icon' => 'shield-alt'],
                        ['label' => 'تاريخ آخر ترقية عسكرية',   'date' => $trainee->current_promotion_date, 'icon' => 'chevron-double-up'],
                    ];
                @endphp
                ...
                {{-- صندوق الملاحظات التدريبية --}}
                <div class="bg-[#070B14]/40 border border-[#1e293b] p-4 rounded-xl space-y-2">
                    <span class="text-xs font-bold text-gray-400 block mb-1">ملاحظات لجنة التدريب:</span>
                    <p class="text-xs text-gray-500 leading-relaxed bg-[#070B14] p-3 rounded-lg border border-[#1e293b]/60 min-h-[75px]">
                        {{ $trainee->notes ?? 'لا توجد ملاحظات تدريبية أو إدارية مرصودة بملف المتدرب الحالي.' }}
                    </p>
                </div>
            </div>
        ...
        {{-- ===== قسم الدورات والبرامج التخصصية للمتدرب ===== --}}
        <div class="space-y-5 pt-6 border-t border-[#1e293b]">
            <h3 class="text-xs font-bold text-gray-400 flex items-center gap-2 tracking-wide">
                ...
                <span>سجل الحقائب التدريبية والدورات التخصصية الحاصل عليها المتدرب</span>
            </h3>

            @if($trainee->courses && $trainee->courses->count() > 0)
                            ...
                            <div class="text-[10px] text-gray-400 space-y-2 font-mono">
                                <div class="flex justify-between flex-row-reverse">
                                    ...
                                    <span class="text-gray-300 font-bold">
                                        {{ $course->pivot->start_date ? \Carbon\Carbon::parse($course->pivot->start_date)->format('Y-m-d') : '---' }}
                                    </span>
                                </div>
                                <div class="flex justify-between flex-row-reverse">
                                    <span class="font-sans text-gray-500">منفذ الدورة</span>
                                    <span class="truncate max-w-[90px] text-gray-300 font-semibold">{{ $course->pivot->conducting_entity ?? '---' }}</span>
                                </div>
                                <div class="flex justify-between flex-row-reverse">
                                    <span class="font-sans text-gray-500">رقم الشهادة</span>
                                    <span class="text-gray-300 font-bold">{{ $course->pivot->cert_number ?? '---' }}</span>
                                </div>
                            </div>
                            <div class="pt-1">
                                <span class="inline-flex items-center gap-1 text-[9px] text-emerald-400 bg-emerald-500/5 border border-emerald-500/10 px-2.5 py-0.5 rounded-full font-medium">
                                    <span class="w-1 h-1 rounded-full bg-emerald-500"></span> شهادة معتمدة
                                </span>
                            </div>
                        </div>
                    @endforeach
            ...
            @else
                {{-- لوحة التنبيه عند خلو ملف المتدرب من الدورات --}}
                <div class="flex flex-col items-center justify-center p-8 bg-[#070B14]/40 border border-dashed border-[#1e293b] rounded-xl text-center space-y-3">
                    ...
                    <div class="space-y-1">
                        <p class="text-sm font-bold text-gray-300">لا توجد دورات مسجلة بملف المتدرب</p>
                        <p class="text-xs text-blue-400 font-semibold max-w-md mx-auto">
                            " المتدرب: {{ $trainee->full_name }} "
                        </p>
                        <p class="text-[11px] text-gray-500">لم يتم رصد أو ربط أي حقائب تدريبية أو دورات تخصصية بهذا القيد العسكري حتى تاريخه.</p>
                    </div>
                </div>
            @endif
        </div>
